...
img_1 = cv2.imread('photo/weapon_498x501.png', -1)
img_2 = cv2.imread('photo\magic_circle_2.png', -1)
...
while True:
    ...
    hand_result=hands.process(rgbimg)
    ...
    if face_results.multi_face_landmarks:
                    _, left_eye_status = isOpen(img, face_results, 'LEFT EYE', 
                                        threshold=4.5 , display=False)
                    _, right_eye_status = isOpen(img, face_results, 'RIGHT EYE', 
                                         threshold=4.5, display=False)
                    for face_landmarks in face_results.multi_face_landmarks:
                        mp_drawing.draw_landmarks(image=img, landmark_list=face_landmarks, connections=mp_face_mesh.FACEMESH_CONTOURS, landmark_drawing_spec=drawing_spec, connection_drawing_spec= drawing_spec)
                        for face_num, face_landmarks in enumerate(face_results.multi_face_landmarks):
                            if left_eye_status[face_num] == 'OPEN':  
                                img = overlay(img, left_eye, face_landmarks,
                                'LEFT EYE', mp_face_mesh.FACEMESH_LEFT_EYE, display=False)
                            if right_eye_status[face_num] == 'OPEN':
                                img = overlay(img, right_eye, face_landmarks,
